...
export class UserContestQuestionComponent {
  ...
  ngOnInit(): void {
    ...
    this.submitProblemService.questionAceepted.subscribe((questionId:number)=>{
  ...
  getAllContestQuestion(url: string) {
    this.problemService.fetchAllContestQuestion(url).subscribe(
      (data) => {
        ...
        console.warn("problem count", this.problemService.getContestProblemCount(), typeof(this.problemService.getContestProblemCount()))
      },
      (error) => {
  ...
  getContestDetail(contestId:string|null){
    this.contestService.getContestById(contestId).subscribe((data)=>{
  ...
  questionTextStatus(){
    if(this.acceptedQuestion!=null && this.acceptedQuestion!=0){return 'accepted-text-color'}
